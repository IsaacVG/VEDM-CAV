...
map<int, double> PreviousVehAcc;
map<int, int> VehTargetLane;
map<int, int> VehTag;
map<int, int> VehToAdjustDecForCutIn;
map<int, int> CACCLeader;
map<int, int> HowManyFollwers;
//map<int,int> VehInGroup;
	...
	if ((VehTargetLane[current_veh] == current_lane || VehTargetLane[current_veh] == 0) && lateral_pos_ind == 0.0)
	{
		...
		if (LeadingVeh_ID > 0 && DistLeadingVeh < MaxLookAheadDist)
		{
			...
			{   // no CACC vehicle ahead; it works as a ACC with 1.2 seconds of headway 
				...
				int followers = 0;
				for (map<int, int>::iterator it = CACCLeader.begin(); it != CACCLeader.end(); ++it)
				{
